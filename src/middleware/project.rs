...
#[profiling::function]
pub fn read_project(path: &Path, parent_context: Option<&Context>, vfs: &Vfs) -> Result<Snapshot> {
	...
	// A nested project file without its own `scope` field inherits the
	// serving root's scope instead of re-defaulting to code (Design §7.0)
	if project.scope.is_none() {
		if let Some(parent) = parent_context {
			meta.context.set_scope(parent.scope());
		}
	}
...
pub fn new_snapshot_node(
	...
	if let Some(path_node) = node.path {
		...
		if vfs.exists(&path) {
			...
			wsync_warn!(
				"Path specified in the project does not exist: {}. Please create this path and restart WSync \
