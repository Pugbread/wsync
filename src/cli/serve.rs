...
use crate::{
	...
	sessions, wsync_error, wsync_info, wsync_warn,
};
...
pub struct Serve {
	...
	/// Run WSync asynchronously
	#[arg(short = 'A', long = "async")]
	run_async: bool,

	/// Spawn the WSync child process (internal)
	#[arg(long, hide = true)]
	wsync_spawn: bool,
}
...
impl Serve {
	pub fn main(self) -> Result<()> {
		...
		if !self.wsync_spawn && (self.run_async || config.run_async) {
			return self.spawn();
		}
		...
		if !server::is_port_free(&host, port) {
			if config.scan_ports {
				let max_port = config.port_scan_max.max(port);

				if let Some(new_port) = server::get_free_port(&host, port, max_port) {
					wsync_warn!(
					...
					bail!(
						"No free port found in range {}-{}! Increase the {} setting to scan further",
						port.to_string().bold(),
						max_port.to_string().bold(),
						"port_scan_max".bold()
					);
				}
			} else {
		...
		if let Some(path) = sourcemap_path {
			...
			wsync_info!("Generated sourcemap at: {}", path.to_string().bold());
			...
			thread::spawn(move || loop {
				...
				match core.sourcemap(Some(path.clone()), false) {
					Ok(()) => (),
					Err(err) => {
						wsync_error!("Failed to regenerate sourcemap: {}", err);
					}
				}
			});
		}
		...
		let server = Server::new(core, &host, port, ServerIdentity::unmanaged());
		...
		// Graceful stop (`/stop` route): remove this session's entry
		if let Ok(Some(session)) = sessions::get(None, Some(host.clone()), Some(port)) {
	...
	fn spawn(self) -> Result<()> {
		...
		Program::new(ProgramName::Wsync).args(args).spawn()?;
