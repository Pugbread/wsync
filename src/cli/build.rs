...
impl Build {
	pub fn main(self) -> Result<()> {
		...
		if self.watch && !self.wsync_spawn && (self.run_async || config.run_async) {
			return self.spawn();
		}
		...
		// Scope governs sync, never packaging: a build that silently drops
		// non-code files (msgpack modules, toml manifests, models) ships a
		// broken artifact, so builds always run the full projection
		project.scope = Some(String::from("full"));
		...
		if use_ts {
			wsync_info!("Compiling TypeScript files..");
		...
		if self.watch {
			...
			wsync_info!("Watching for changes..");
