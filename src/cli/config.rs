...
use crate::{
	config::{Config as WsyncConfig, ConfigKind},
	ext::PathExt,
	logger, util, wsync_info,
};
...
impl Config {
	pub fn main(self) -> Result<()> {
		let config = WsyncConfig::new();

		let config_kind = match self.config.unwrap_or_default() {
			ConfigType::Default => ConfigKind::Default,
			ConfigType::Global => ConfigKind::Global(util::get_wsync_dir()?.join("config.toml")),
			ConfigType::Workspace => ConfigKind::Workspace(env::current_dir()?.join("wsync.toml")),
		};
		...
		{
			drop(config);
			WsyncConfig::load_virtual(config_kind)?;
		} else {
			drop(config);
		};
		...
		if self.list {
			wsync_info!(
				...
				config.list(),
				"https://github.com/Pugbread/wsync".bold()
			);
		...
		match (self.setting, self.value) {
			(Some(setting), Some(value)) => {
				drop(config);
				let mut config = WsyncConfig::new_mut();
			...
			(Some(setting), None) => {
				let default = WsyncConfig::default();
			...
			_ => {
				...
				wsync_info!("Opened config file. Manually go to: {}", config_path.to_string().bold());
