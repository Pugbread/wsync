...
pub fn init(workspace: WorkspaceConfig) -> Result<()> {
	let template_dir = util::get_wsync_dir()?.join("templates").join(workspace.template);
...
pub fn init_ts(workspace: WorkspaceConfig) -> Result<Option<PathBuf>> {
	...
	wsync_info!("Waiting for {}..", package_manager.bold());
	...
	let template_dir = util::get_wsync_dir()?.join("templates").join(template);
...
fn add_license(path: &Path, license: &str, fallback: &str) -> Result<()> {
	...
	let license_template = || -> Result<String> {
		match Client::new().get(url).header(USER_AGENT, "WSync").send() {
			Ok(response) => {
	...
	match license_template {
		...
		Err(err) => {
			...
			wsync_warn!("Failed to add license: {}. Using basic fallback instead!", err);
