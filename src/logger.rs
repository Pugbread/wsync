...
// These WSync logs ignore verbosity level, aside of `Off`
#[macro_export]
macro_rules! wsync_error {
    ($($arg:tt)+) => (log::log!(target: "wsync_log", log::Level::Error, $($arg)+))
}

#[macro_export]
macro_rules! wsync_warn {
    ($($arg:tt)+) => (log::log!(target: "wsync_log", log::Level::Warn, $($arg)+))
}

#[macro_export]
macro_rules! wsync_info {
    ($($arg:tt)+) => (log::log!(target: "wsync_log", log::Level::Info, $($arg)+))
}
...
pub fn init(verbosity: LevelFilter, log_style: WriteStyle) {
	...
	builder.format(move |buffer, record| {
		if record.level() > verbosity && record.target() != "wsync_log" {
			return Ok(());
		}
	...
	builder.filter_module("axum", LevelFilter::Warn);
