...
use crate::{logger, util, wsync_error};
...
pub fn hook() {
	panic::set_hook(Box::new(|panic_info| {
		...
		let mut report = String::from("> This crash report was automatically generated by WSync");
		report.push_str("\n\nDetails:\n--------\n");
		report.push_str("*Provide extra details about your problem or what happened here*");

		wsync_error!("{}", "WSync has crashed!".bold());

		report.push_str("\n\nMessage:\n----------\n");
		report.push_str(message);

		wsync_error!("{}: {}", "Message".bold(), message);
		...
		if let Some(location) = panic_info.location() {
			...
			wsync_error!("{}: {}: {}", "Location".bold(), location.file(), location.line());
		} else {
		...
		if verbosity != LevelFilter::Trace {
			report.push_str("Verbose output disabled, run WSync with `-vvvv` flag to enable");

			wsync_error!(
				"{}: Run WSync with {} flag to show detailed tracing",
				"Verbosity".bold(),
				"-vvvv".bold()
			);
		} else {
		...
		if backtrace_enabled {
			...
			report.push_str("Backtrace disabled, run WSync with `--backtrace` flag to enable");

			wsync_error!(
				"{}: Run WSync with {} flag to show full backtrace\n",
				"Backtrace".bold(),
				"--backtrace".bold()
			);
		}
		...
		if report_issue {
			...
			url.push_str(&format!("/issues/new?title=WSync crashed: {message}&body="));
