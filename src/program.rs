...
use crate::{config::Config, ext::WriteStyleExt, logger, util, wsync_error};
...
impl Program {
	...
	fn get_command(&self) -> Command {
		if self.program == ProgramName::Wsync {
			let mut command = Command::new(env::current_exe().unwrap_or(PathBuf::from("wsync")));
		...
		let mut program = match (&self.program, package_manager) {
			...
			(ProgramName::Wsync, _) => unreachable!(),
		}
		.to_owned();
	...
	fn handle_error<T>(&self, error: Error) -> Result<Option<T>> {
		if error.kind() == ErrorKind::NotFound {
			wsync_error!("{}", self.get_error(&self.message));
