...
use crate::{logger::Table, util, wsync_error};
...
pub struct Config {
	...
	/// Run WSync asynchronously, freeing up the terminal
	pub run_async: bool,
	...
	/// Check for new WSync releases on startup
	pub check_updates: bool,
	/// Automatically install WSync updates if available
	...
	/// Share anonymous WSync usage statistics (currently local counters only)
	// TODO(phase-7): decide whether WSync ships telemetry at all; the Argon
	// upload endpoint was removed in the fork so this flag gates nothing yet
	pub share_stats: bool,

	/// Emit .model.json instead of folder with .data.json when syncing back new leaf instances
	// TODO(phase-4): wire into the syncback write middleware fallback
	pub syncback_model_json: bool,
	/// Detect conflicting changes between Studio and the file system instead of applying the last one
	// Gates the baseline conflict engine (core::conflict, Design §6.3): off
	// means exactly the engine-less last-writer-wins behavior incl. the
	// changes_threshold prompt
	pub conflict_engine: bool,
	/// Serve the legacy Argon HTTP protocol so a stock Argon plugin can connect
	// NOTE(phase-1): the Argon-protocol HTTP endpoints are served regardless
	// of this flag until the WSync WebSocket protocol lands in phase 2
	pub compat_argon: bool,
	/// Highest port checked when scanning for a free or discoverable port
	pub port_scan_max: u16,
	/// Route connect-time divergence prompts to the desktop app when running
	// TODO(phase-6): consumed by the desktop app divergence modal flow
	pub auto_open_app_modal: bool,
...
impl Default for Config {
	fn default() -> Self {
		Self {
			...
			syncback_model_json: false,
			conflict_engine: true,
			compat_argon: false,
			port_scan_max: 7990,
			auto_open_app_modal: true,
...
impl Config {
	...
	pub fn load() -> Result<ConfigKind> {
		...
		let config_kind = || -> Result<ConfigKind> {
			let workspace_config = env::current_dir()?.join("wsync.toml");
			let global_config = util::get_wsync_dir()?.join("config.toml");
	...
	pub fn load_virtual(kind: ConfigKind) -> Result<()> {
		let kind = match kind {
			ConfigKind::Default => ConfigKind::Global(util::get_wsync_dir()?.join("config.toml")),
			_ => kind,
		};
	...
	pub fn load_workspace(path: &Path) {
		Self::load_specific(ConfigKind::Workspace(path.join("wsync.toml")))
	}
	...
	fn load_specific(kind: ConfigKind) {
		...
		match load_result {
			...
			Err(err) => {
				wsync_error!("Failed to load {} config file: {}", kind.to_string().bold(), err);
			}
		}
	}
