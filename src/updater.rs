...
pub fn get_status() -> Result<UpdateStatus> {
	let path = util::get_wsync_dir()?.join("update.toml");
...
fn update_cli(prompt: bool, force: bool) -> Result<bool> {
	...
	if bump_is_greater(current_version, &release.version)? || force {
		if !prompt
			|| logger::prompt(
				&format!(
					"New WSync version: {} is available! Would you like to update?",
					release.version.bold()
				),
				true,
			) {
			if !prompt {
				wsync_info!("New WSync version: {} is available! Updating..", release.version.bold());
			}
			...
			match update.update() {
				Ok(_) => {
					wsync_info!(
						...
						"https://github.com/Pugbread/wsync/releases".bold()
					);
					return Ok(true);
				}
				Err(err) => wsync_error!("Failed to update WSync: {}", err),
			}
		} else {
			trace!("WSync is out of date!");
		}
	} else {
		trace!("WSync is up to date!");
	}
...
fn update_plugin(status: &mut UpdateStatus, prompt: bool, force: bool) -> Result<bool> {
	...
	if bump_is_greater(current_version, &release.version)? || force {
		if !prompt
			|| logger::prompt(
				&format!(
					"New version of WSync plugin: {} is available! Would you like to update?",
					release.version.bold()
				),
				true,
			) {
			if !prompt {
				wsync_info!(
					"New version of WSync plugin: {} is available! Updating..",
					release.version.bold()
				);
			}
			...
			match update.download() {
				...
				Err(err) => wsync_error!("Failed to update WSync plugin: {}", err),
			}
		} else {
			trace!("WSync plugin is out of date!");
		}
	} else {
		trace!("WSync plugin is up to date!");
	}
...
fn update_templates(status: &mut UpdateStatus, prompt: bool, force: bool) -> Result<bool> {
	if status.templates_version < TEMPLATES_VERSION || force {
		if !prompt || logger::prompt("Default templates have changed! Would you like to update?", true) {
			if !prompt {
				wsync_info!("Default templates have changed! Updating..",);
			}
