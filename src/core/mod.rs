...
impl Core {
	...
	pub fn new(project: Project, watch: bool) -> Result<Self> {
		...
		let conflicts = Arc::new(ConflictEngine::new());
	...
	pub fn conflicts(&self) -> Arc<ConflictEngine> {
		self.conflicts.clone()
	}

	/// The core's VFS. Bulk phases use it to fence (pause/resume) the watcher
	/// and to re-read swapped roots through the same backend the processor
	/// uses (Design §7.4-A)
