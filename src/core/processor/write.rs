...
pub fn apply_addition(snapshot: AddedSnapshot, tree: &mut Tree, vfs: &Vfs) -> Result<()> {
	...
	// Code scope (Design §7.0): classes outside the projection are
	// Studio-authoritative — never written to disk, skipped without error
	if !parent_meta.context.scope().allows_class(&snapshot.class) {
		debug!(
			"Skipping addition of {} ({}): class is outside the code-scope projection",
			snapshot.name, snapshot.class
		);
		return Ok(());
	}
	...
	fn write_instance(
		...
		// Properties always ride along: for a `Script` the middleware consumes
		// RunContext and encodes it in the file suffix
		if let Some(middleware) = Middleware::from_class(&snapshot.class, Some(&mut properties)) {
			...
			// Leftover properties the file cannot carry (attributes, tags,
			// RunContext.Plugin) go to the data sidecar in every scope —
			// `write_data` writes nothing when there is nothing left
			{
	...
	fn add_non_project_instances(
		...
		// Code scope: nested out-of-projection children are skipped whole
		// (their subtrees stay Studio-authoritative)
		if !parent_meta.context.scope().allows_class(&snapshot.class) {
			debug!(
				"Skipping addition of {} ({}): class is outside the code-scope projection",
				snapshot.name, snapshot.class
	...
	fn add_project_instances(
		...
		if !parent_meta.context.scope().allows_class(&snapshot.class) {
			debug!(
				"Skipping project-node addition of {} ({}): class is outside the code-scope projection",
				snapshot.name, snapshot.class
			);
			return;
		}
...
pub fn apply_update(snapshot: UpdatedSnapshot, tree: &mut Tree, vfs: &Vfs) -> Result<()> {
	...
	if let Some(instance) = tree.get_instance(snapshot.id) {
		let context = &tree.get_meta(snapshot.id).unwrap().context;
		let filter = context.syncback_filter();

		// Code scope: instances of out-of-projection classes never reach the
		// tree, but fail closed if one does — they are Studio-authoritative
		if !context.scope().allows_class(&instance.class) {
			debug!(
				"Skipping update of {} ({}): class is outside the code-scope projection",
	...
	fn update_non_project_properties(
		...
		// Properties always ride along: for a `Script` the middleware consumes
		// RunContext and encodes it in the file suffix
		if let Some(middleware) = Middleware::from_class(&instance.class, Some(&mut properties)) {
			let new_path = {
				let mut paths = meta
					.context
					.sync_rules_of_type(&middleware, true)
			...
			if let Some(file_path) = file_path {
				...
				// Leftovers the file cannot carry go to the data sidecar in
				// every scope; `write_data` writes nothing for nothing
