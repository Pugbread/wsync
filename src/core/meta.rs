...
use crate::{
	config::Config,
	constants::{default_sync_rules, RUNTIME_DIRS},
	ext::PathExt,
	glob::Glob,
	middleware::Middleware,
	project::{Project, ProjectNode, Scope},
};
...
pub struct Context {
	...
	/// The project's sync scope (Design §7.0). Placeholder contexts default
	/// to `Full` (today's behavior); every project-derived context carries
	/// the project's real scope and children inherit it
	scope: Scope,
}
...
impl Context {
	...
	/// Overrides the scope (nested project files without their own `scope`
	/// field inherit the serving root's scope instead of re-defaulting)
...
impl Meta {
	...
	pub fn from_project(project: &Project) -> Self {
		...
		// WSync's own runtime directories (backups, artifacts, workflows) are
		// always ignored — their churn must never re-enter the sync pipeline
		// or leak into snapshots (Design §12)
		let mut ignore_globs = project.ignore_globs.clone();
		...
		let context = Context {
			...
			ignore_rules: IgnoreRule::from_globs(ignore_globs, project.workspace_dir.clone()),
			syncback_filter,
			scope: project.scope(),
		};
